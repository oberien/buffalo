...
IsKeepAliveCondition()
{
  ...
  RET=0
  DOWNLOAD_BYTES=`grep eth1 /proc/net/dev | awk '{print $2}'`
  UPLOAD_BYTES=`grep eth1 /proc/net/dev | awk '{print $10}'`
  if [ "$LAST_BYTES_CHECK" -ne 0 ] ; then
    # get bytes per sec since last check
    T=`date +%s`
    DIFF=$(($T - $LAST_BYTES_CHECK))
    D_DOWNLOAD=$(($DOWNLOAD_BYTES - $OLD_DOWNLOAD_BYTES))
    D_UPLOAD=$(($UPLOAD_BYTES - $OLD_UPLOAD_BYTES))
    DOWNSTREAM=$(($D_DOWNLOAD / $DIFF))
    UPSTREAM=$(($D_UPLOAD / $DIFF))
    if [ "$DOWNSTREAM" -ge 1000 -o "$UPSTREAM" -ge 1000 ] ; then
      RET=$KAC_NETW
    fi
  fi
  OLD_DOWNLOAD_BYTES=${DOWNLOAD_BYTES}
  OLD_UPLOAD_BYTES=${UPLOAD_BYTES}
  LAST_BYTES_CHECK=`date +%s`
...
CheckStatus()
{
  log "CheckStatus"
	...
	if [ "${SW_STAT}" = "on" -a "${PW_STAT}" = 1 ] ; then
		...
		IsKeepAliveCondition
		KEEP_ALIVE_CONDITION=$?
    log "KAC: $KEEP_ALIVE_CONDITION"
    log "Old KAC: $OLD_KEEP_ALIVE_CONDITION"
		if [ ${KEEP_ALIVE_CONDITION} -gt 0 ] ; then
